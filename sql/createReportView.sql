CREATE OR REPLACE VIEW pum_project_throughput_view AS
  SELECT proj.id AS project_id, proj.title AS project_name, proj.start_date, proj.end_date, proj.projectmanager_id,
    ...
    proj.date_first_main, proj.date_expert_added, proj.date_expert_reacted, proj.date_cv_sent, proj.date_cust_approves_expert,
    proj.date_start_logistics, proj.anamon_id AS anamon_id, proj.sector_coordinator_id AS sector_coordinator_id,
    proj.country_coordinator_id AS country_coordinator_id, proj.project_officer_id AS project_officer_id,
    proj.programme_id, proj.country_id, proj.customer_id,
